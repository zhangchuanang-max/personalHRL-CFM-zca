...
### 2.2 异构机器人定义
> ⚠️ **已知 Bug**：当前代码中 `task_env.py` 第 232 行存在硬编码错误，所有机器人的速度都被设置为 `0.2`，导致上述差异化速度未生效。需要将 `'velocity': 0.2` 修改为 `'velocity': current_vel` 以启用差异化速度。
...
相比原始 CFM 论文，本项目已完成以下空间站场景改造：

✅ **场景建模**
- 三区域空间拓扑（Zone A/B/C）
- 三种固定角色机器人（Inspector/Manipulator/Sentinel）
- 三类分级任务（单人/双人/三人协作）
...
🔲 **创新点 3（部分）：物理可行性约束**
- Zone 准入权限的硬约束（当前仅有位置分布，无逻辑限制）
- 任务容量上限检查
- Deadline 可达性预判

⚠️ **已知 Bug**
- 机器人差异化速度未生效（task_env.py:232）

###  Bug 修复记录

| Bug | 位置 | 问题 | 修复 |
|-----|------|------|------|
| 速度硬编码 | `task_env.py` 第232行 | `'velocity': 0.2` 所有机器人一样 | → `'velocity': current_vel` |
| float 转换错误① | `task_env.py` 第203行 | `float(tasks_time[i, :])` 数组无法转标量 | → `float(tasks_time[i, 0])` |
| float 转换错误② | `task_env.py` 第575行 | `float(np.max(arrival, keepdims=True))` | → `float(np.max(arrival))` |
| Greedy 永远选 depot | `task_env.py` execute_greedy_action | depot 距离=0，总被优先选中 | → 跳过 depot(index 0)，优先选任务 |
| Greedy 时间不推进 | `task_env.py` execute_greedy_action | 所有agent被block后无限循环 | → 添加时间强制推进逻辑 |
| 已开工任务被加入 | `task_env.py` agent_step | agent加入已feasible的任务不更新状态 | → 转为回depot |
...
1. **修复速度 Bug**（5 分钟）
2. **实现 Zone 准入约束**（1-2 天）
3. **完善 CFM cycle-back 机制**（2-3 天）
4. **实现任务容量与 Deadline 预判**（1-2 天）
5. **双层架构改造**（1-2 周，最复杂）
...
| 机器人速度 | 统一 | **差异化速度**：Type1=0.5 / Type2=0.1 / Type3=0.3 |
| 奖励函数 | 基于 makespan | 增加**失败惩罚项**（-200/task），强制 RL 关注成功率 |
| 动作掩码 | 技能匹配 | 新增 **Zone 准入 + 容量上限 + Deadline 预判** |
| 决策顺序 | 顺序决策 | 新增 **CFM cycle-back**（deferred_agents 机制） |
